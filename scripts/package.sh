...
# the files below must sit at the archive root — nested under a directory,
# the plugin silently fails to load. This only packages the SP plugin half
# of this repo (manifest.json/plugin.js/index.html/icon.svg); mcp_server.py
# is a separate component installed via setup.sh/setup.bat, not shipped here.
set -euo pipefail
